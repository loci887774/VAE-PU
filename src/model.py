...
class VAEdecoder(tf.keras.Model):
    ...
    def __init__(self, config):
        ...
        elif 'lamda' in self.config['data']:
            self.vae_de.add(tf.keras.layers.Dense(self.config['n_input']))
        '''
...
class myPU():
    ...
    def pretrain(self, x_pl, x_u):
        ...
        with tf.GradientTape() as vae_en_tape, tf.GradientTape() as vae_de_tape:
            ...
            if 'MNIST' in self.config['data']:
                ...
                # Dữ liệu nhị phân -> Dùng Sigmoid Cross Entropy
                recon_x = self.model_de.decode(h_y, h_o)
                loss = tf.reduce_mean(tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=x, logits=recon_x), axis=1))
    ...
    def train_step_vae(self, x_pl, x_u, epoch):
        ...
        with tf.GradientTape() as vae_en_tape, tf.GradientTape() as vae_de_tape, tf.GradientTape() as cl_tape, tf.GradientTape() as param_tape:
            ...
            c0 = tf.reduce_sum(c0, axis=1, keepdims=True)
            c1 = tf.reduce_sum(c1, axis=1, keepdims=True)
            ...
            loss1 = self.config['alpha_mi'] * tf.reduce_mean(tf.multiply(1 - c, loss1_0) + tf.multiply(c, loss1_1))
            ...
            loss7 = - self.config['alpha_cl'] * tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=label, logits=c_o))
    ...
    def generate(self, x_pl, x_u, mode='near_o'):
        if mode == 'near_o':
            ...
            # --- ƯU TIÊN KIỂM TRA LAMDA TRƯỚC ---
            if 'lamda' in self.config['data']:
                # Dữ liệu nhị phân -> Cần sigmoid=True để ép về [0,1]
                x_pu = self.model_de.decode(h_y, ne_h_o, sigmoid=True)
            # ------------------------------------
    
            elif '20' not in self.config['data']:
            # Dòng này rất nguy hiểm nếu tên file có chứa số '20' (ví dụ '2013')
            # Nhưng vì ta đã catch 'lamda' ở trên rồi nên an toàn.
                x_pu = self.model_de.decode(h_y, ne_h_o, sigmoid=True)
    ...
    def accuracy(self, dataset):
        ...
        # --- SỬA ĐOẠN NÀY (Thêm if/else cho acc) ---
        total = tp + tn + fp + fn
        if total == 0:
            acc = 0
        else:
