...
def train(num_exp, model_config, pretrain=True):
    ...
    #path_kaggle = '/kaggle/input/lamda2013/'
    #path_kaggle = '/kaggle/input/datasets/lamthicucb2203709/lamda2013/'
    #path_kaggle = '/kaggle/input/datasets/lamthicucb2203709/pu-data/'
    ...
    opt_en = tf.keras.optimizers.Adam(learning_rate=model_config['lr_pu'])
    opt_de = tf.keras.optimizers.Adam(learning_rate=model_config['lr_pu'])
    opt_disc = tf.keras.optimizers.Adam(learning_rate=model_config['lr_disc'], beta_1=model_config['beta1'], beta_2=model_config['beta2'])
    opt_cl = tf.keras.optimizers.Adam(learning_rate=model_config['lr_pu'])
    opt_pn = tf.keras.optimizers.Adam(learning_rate=model_config['lr_pn'])
    
    ##### v2.0 [201122] #####
    # if you want to train the parameters (pi, mu, sigma)
    # cái này là tf1.x rồi, xưa rồi Diễm =)))
    #opt_param = tf.contrib.optimizer_v2.AdamOptimizer(learning_rate=model_config['lr_pu'])
    opt_param = tf.keras.optimizers.Adam(learning_rate=model_config['lr_pu'])
    ##### v2.0 [201122] #####
    ...
    # ==============================================================================
    # 🟢 BƯỚC 1: THÊM ĐOẠN CODE LOAD CHECKPOINT VÀO ĐÂY !!!!!!!!!!!!!!!!!!!!!!!!!
    # ==============================================================================
    # print("------------------------------------------------------")
    # if manager.latest_checkpoint:
    #     checkpoint.restore(manager.latest_checkpoint)
    #     print("✅ Đã tìm thấy và khôi phục checkpoint từ: {}".format(manager.latest_checkpoint))
    #     print("🚀 Sẽ bắt đầu chạy tiếp từ Epoch 401...")
    #     start_epoch = 401  # <--- QUAN TRỌNG: Bắt đầu từ 401
    # else:
    #     print("⚠️ Không tìm thấy checkpoint nào. Chạy lại từ đầu (Epoch 1).")
    #     start_epoch = 1
    # print("------------------------------------------------------")
    # ==============================================================================
    ...
    # CHẠY TỪ EPOCH 400 TRONG CHECKPOINT
    #for epoch in range(start_epoch, model_config['num_epoch'] + 1):
    ...
    plotLoss(lstLoss3, model_config['directory'] + 'loss_disc.png')
    plotLoss(lstLoss2, model_config['directory'] + 'loss_gen.png')
    plotLoss(lstLoss4, model_config['directory'] + 'loss_cl.png')
    plotLoss(lstVal, model_config['directory'] + 'loss_val.png')
